...
startVoltage = rospy.get_param("~startVoltage", default=30)
...
def CamFileNamePrefix(strValue):
    ...
    try:
        ...
        responseResult = setbool_proxy(strValue,20)
    except Exception as e:
        message = traceback.format_exc()
        rospy.loginfo(message)
    return responseResult
...
def CheckSafetyLidar(moveRange = None):
    ...
    #여기에 45를 더 할 것
    camAngelOffset = 45
    checkAngle = list(range(40+camAngelOffset, 56+camAngelOffset))
        ...
        if angle in checkAngle:
            ...
            if distance < finalRange:
                ...
                if not runFromLaunch:
                    ...
                    message = f'moveLength : {moveLength}, curV = {curV}, Lidar Info = {sorted(dicLD06.items())} from {sys._getframe(1).f_code.co_name}'
                    rospy.loginfo(message)
                return False
    lockLD06.release()
    LastMoveRange = param_VMax_Limit
    ...
    try:
        while len(CMD_Queue) > 0:
            ...
            if drvID == drvCaption_H and len(valueList) > 7 and chkStatusH == strREADY: 
                ...
                if float(UIN5V) < safetyVoltage and not IsDockingH() and len(df_fromSPGMAP) > 5: #테스트용 코드-_- 왔다갔다 지시정보 돌리다가 33볼트 아래로 떨어지면 가동되겠지 뭐;
                    ...
                    if sStart != 'RESET':
                        ...
                        msgErr = f'현재전압:{UIN5V}V 이 안전전압 {safetyVoltage}V 보다 낮습니다. 스마트 충전을 시작합니다.'
                ...
                if IsDockingH(): #디버그용-_-
                    if float(UIN5V) < startVoltage:
                        msgErr = f'현재노드인덱스:{currOperationPos},현재전압:{UIN5V}V 이 안전전압 {startVoltage}V 보다 낮아 출발할 수 없습니다.'
...
def motorStop(drvID, bSmooth):
    ...
    rospy.loginfo(f'Stop OK : Check Safety : {CheckSafetyLidar()}, Last Lidar Data : {lastLD06Time}')
    return True
...
def motorMove(Range, Spd, Dir, isAbsolutePos,iACC : str,  iDECC : str):
    ...
    if iACC != None and iACC.isdecimal():
        accTime = int(iACC)
    if iDECC != None and iDECC.isdecimal():
        decTime = int(iDECC)
    # if iACC != None and iACC !='0':
    #     accTime = int(iACC)
    # if iDECC != None and iDECC !='0':
    #     decTime = int(iDECC)
    ...
    if chkAlarm(EncoderID): #체크 알람은 추후 구현. 현재는 알람이 없으면 1, 알람이 있으면 0 리턴, 에러코드 체크로 바꾸면 될 듯.
        ...
        try:
            ...
            if EncoderID == drvCaption_V:
                ...
                chkStatusH,chkStatusV = getMultiEx(dic_485,'TARG')
                rospy.loginfo(f'서보운행현황 : {chkStatusH},{chkStatusV},{currOperationPos}')
    ...
    try:
        ...
        if servo_keepalive <= servo_keepalive_MAX:
            ...
            if td.total_seconds()  > 1:
                ...
                if curLocH != '0' and isTimeExceeded(lastActionTime, param_Return_Time*1000*60) and not isOperation() and not IsDockingH():
                    #엔코더값 확인
        ...
        #Tmp = rospy.get_param(nameof(param_BMS_Show))
            ...
            #자동주행 운행부분
            if (chkStatusH == strREADY) and strREADY == chkStatusV: #경로정보대로 운행시
                if not suspend  and len(CMD_Queue) ==0: #회피기동중인 경우 대기한다.
                    ...
                    if len_df_fromSPGMAP > currOperationPos and cmdIDCurrent == '':
                        ...
                        sTier = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.TIER.name,'')
                        sStart = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.START.name,'')
                        sLastSeen = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.LASTSEEN.name,'')
                        sLENGTH = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.LENGTH.name,'')
                        sSPD = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.SPD_H.name,'')
                        sMOVE_ = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.MOVE_.name,'')
                        
                        sACC = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.ACC.name,None)
                        sDECC = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.DECC.name,None)
                        ...
                        sSCAN_3D = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.SCAN_3D.name,'')

                        # ''' STOPWAVE 해당 구간에서는 사진촬영을 연사로 진행한다 '''
                        # ''' 20221118 연사가 아닌 단발로 바꾸자 '''
                        sMULTI_CAM = df_fromSPGMAP.loc[currOperationPos].get(SPG_Keys.MULTI_CAM.name,'')
                        ...
                        sMULTI_CAM_Prev = sSCAN_3D_Prev = 'False'
                        ...
                        if currOperationPos > 0 :
                            ...
                            #sMULTI_CAM_Prev = df_fromSPGMAP.loc[currOperationPos-1].get(SPG_Keys.MULTI_CAM.name)
                            #sSCAN_3D_Prev = df_fromSPGMAP.loc[currOperationPos-1].get(SPG_Keys.SCAN_3D.name)
                        ...
                        #     currOperationPos += 1
                        #     SendFeedbackFinish(CmdStatus.Canceled.name,sLastSeen)
                        elif sMOVE_ == dirCaption_D and runFromLaunch and sLENGTH != None and not chkLidar:
                            dicReport[ReportFields.IMG_TRIED.name] += 1
                            dicReport[ReportFields.IMG_SKIPPED.name] += 1
                            currOperationPos += 1
                            SendFeedbackFinish(CmdStatus.Canceled.name,sLastSeen)
                        elif sMOVE_ == dirCaption_D and sLENGTH != None and not chkLidar:
                            print(sMOVE_,sLENGTH)
                            SetSuspend(True)
                        else:
                            cmdIDCurrent = sLastSeen
                            # #데모용 - 방금한 동작이 왼쪽이었다면 틸팅을 한다
                            # if sMovePrev_ == dirCaption_L:
                            #     #TiltDelay(50)
                            #     CamLightEnable(True)
                            #     TiltBox(30,140)
                            #     time.sleep(5)
                            ...
                            #스타트이름이 RESET 이면 사진찍을 필요 없음                            
                            if currOperationPos > 0 and sMovePrev_ == dirCaption_D and curLocV != '0' and sStart != 'RESET':
                                ...
                                # 대영정공 적용된 부분
                                spl = ''
                                #prefixStr = f'{sStartPrev}_{sTierPrev}_{getCurrentDate()}_{getCurrentTime(spl)}_{curLocV_LENGTH}'
                                prefixStr = f'{sStartPrev}_{sTierPrev}_{curLocV_LENGTH}'
                                CamLightEnable(True)
                                #디버그 - 서비스 없으면 예외처리하자.
                            ...
                            if isTrue(sSCAN_3D): #STOPDF 구간에서는 라이다를 동작시키고 ROSBAG을 기록시작한다.
                                LidarEnable(True)

                            SendFeedbackFinish(CmdStatus.Started.name,sLastSeen)
                            
                            # #데모용 - 하강에서는 LED켜고 상승시 끈다
                            # if sMOVE_ == dirCaption_D:
                            #    CamLightEnable(True)
                            #    #TiltBox(105,140)
                            # elif sMOVE_ == dirCaption_U:
                            #    CamLightEnable(False)
                            #    #TiltBox(140,0)
                    ...
                    if IsDockingH():
                        cuV = try_parse_float(UIN5V)
                        sus = safetyVoltage > cuV
                        SetSuspend(sus)
                    # else:
                    #     SetSuspend(not CheckSafetyLidar())
