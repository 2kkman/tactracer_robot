...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        #print(node_CtlCenter_globals.dicModbusCallbackCount)
        try:
            ...
            if (
                lnMap >= 0
                and node_CtlCenter_globals.cmdIdx < lnMap  # cmdIdx 가 lnMap 보다 같거나 크면 지시정보 수행 완료 상태이므로 넘어감
                and not node_CtlCenter_globals.waitCross  # waitCross 면 분기소 동작중이므로 대기.
            ):
                # 동작중인 모터가 없고 동작지시명령 수신부터 0.5초이상 지났을때
                if len(getRunningMotorsBLB()) != -1 and isTimeExceeded(
                    node_CtlCenter_globals.lastCmdTimeStamp, MODBUS_EXECUTE_DELAY_ms
                ):
                    ...
                    try:
                        ...
                        bException = False
                        if len(dicInfo_local) == 0:
                          continue
                      
                        if isinstance(dicInfo_local, dict) and isReadyToMoveH():  # 주행모드
                            # 아르코 마커 인식 클리어
                            ...
                            # 출발노드ID
                            node_CtlCenter_globals.nStart = dicInfo_local[SeqMapField.START_NODE.name]  
                            ...
                            # 도착지 세팅(도착노드ID)
                            node_CtlCenter_globals.nTarget = dicInfo_local[SeqMapField.END_NODE.name]
                            ...
                            node_CtlCenter_globals.dicCrossState = dicInfo_local.get(
                                SeqMapField.CROSS_STATUS.name, {}
                            )
                            ...
                            # # 진행방향 가져오기
                            # node_CtlCenter_globals.nDirection = dicInfo_local.g et(
                            ...
                            if len(node_CtlCenter_globals.dicCrossState) > 0:
                                for (
                                    key,
                                    value,
                                ) in node_CtlCenter_globals.dicCrossState.items():
                                    setNodeStateEx(key, value)
                        ...
                        # 다중제어로 한 리스트에는 복수 모터의 지시정보가 들어있고 동시에 실행시킨다.
                        elif isinstance(dicInfo_local, list) and isFinishedMotor(ModbusID.MOTOR_H):
                            rospy.loginfo(dicInfo_local)
                            if len(dicInfo_local) > 1:
                                lsActivatedMotor = getRunningMotorsBLB()
                                if len(lsActivatedMotor) > 0:
                                    sErrMsg = f'동작중인 모터가 있어 대기합니다:{lsActivatedMotor}'
                                    #raise Exception(sErrMsg)
                                    bException = True
                            
                            if not bException:
                                SendCMD_Device(dicInfo_local)
                            # isScanOn = node_CtlCenter_globals.lsARUCO_Result.get(
                            #     ARUCO_RESULT_FIELD.IS_MARKERSCAN.name, False
                            # )
                            # if not isScanOn:
                            #     CamControl(True)
                            ...
                            #         continue
                            #     # arucoTableID = (
                            #     #     node_CtlCenter_globals.lsARUCO_Result.get(
                            #     #         ARUCO_RESULT_FIELD.MARKER_VALUE.name, -1
                            #     #     )
                            #     # )
                            #     # if node_CtlCenter_globals.node_current not in [
                            #     #     node_CHARGING_STATION,
                            #     #     node_KITCHEN_STATION,
                            #     # ]:
                            #     #     if arucoTableID == -1:
                            #     #         # print('인식된 마커가 없습니다')
                            #     #         sErrMsg = f"인식된 마커가 없습니다:{arucoTableID} - 검색중인 마커 : {node_CtlCenter_globals.node_current}"
                            ...
                            #     #     ):
                            #     #         # print('서비스 대상 테이블 번호와 마커번호가 다릅니다')
                            #     #         sErrMsg = f"서비스 대상 테이블 번호와 마커번호가 다릅니다 - {arucoTableID} - 검색중인 마커 : {node_CtlCenter_globals.node_current}"
                            #     #         # rospy.loginfo(sErrMsg)
                            #     #         raise Exception(sErrMsg)  # 예외를 발생시킴
                            #     #         continue
                            #     #rospy.loginfo(f'{type(dicTmp)},{dicTmp},Aruco:{node_CtlCenter_globals.lsARUCO_Result}')                                
                            ...
                            #     )
                            #     rospy.loginfo(
                            #         f"제어명령어 송신 : {dicTmp}, 현재 위치 : {node_CtlCenter_globals.node_current}"
                            #     )
                            #     SendCMD_Device(cmdCurrent_local)
                        else:
                            bException = True
                            # 진입해서는 안될 부분. 지시정보 생성에 오류발생.
                            #sErrMsg = f"지시정보타입:{type(dicInfo_local)},지시:{dicInfo_local},수평모터정지:{isFinishedMotor(ModbusID.MOTOR_H)}"
                            #rospy.loginfo(sErrMsg)                            
                            # print(type(dicInfo_local, list),isFinishedMotor(ModbusID.MOTOR_H))
                            # rospy.loginfo(sErrMsg)
                            #raise Exception(sErrMsg)  # 예외를 발생시킴
                        # 공통부분
                        ...
                        # pass
                # else:   #동작중인 모터가 0개가 아닐때
                #     if len(node_CtlCenter_globals.activated_motors) > 0:
                #         print(node_CtlCenter_globals.activated_motors)
                ...
                if lnTables > 0 and not node_CtlCenter_globals.waitCross:
                    ...
                    if (
                        ...
                        if node_CtlCenter_globals.is_lifted:
                            if len(getRunningMotorsBLB()) == 0 and isTimeExceeded(
                        ...
                        if (
                            ...
                            if (
                                ...
                                node_CtlCenter_globals.cmdIdx = 0
                                rospy.loginfo(f'ListBLB:{node_CtlCenter_globals.listBLB},ListTable:{node_CtlCenter_globals.listTable}')
                        elif (
                            isTrue(doorStatusOpen_local)
                            and not node_CtlCenter_globals.flag_req_doorClose
                        ):
            ...
            if isActivatedMotor(ModbusID.MOTOR_H.value):
                node_CtlCenter_globals.curBLB_Status = BLB_STATUS_FIELD.MOVING
                if node_CtlCenter_globals.node_target == node_CHARGING_STATION:
                    node_CtlCenter_globals.curBLB_Status = BLB_STATUS_FIELD.HOMING

            dicTimeOutMotor = CheckModbusTimeOut()
            # if len(dicTimeOutMotor):
            #     print()
            # 1초마다 현재 로봇 상태값 송신
            if td_local.total_seconds() >= 1:
                ...
                #print(GetStrArmExtend(1500,1000, 70))
                ...
                node_CtlCenter_globals.lastMainLoopTimeStamp = datetime.datetime.now()
                ...
                if node_CtlCenter_globals.waitCross:
                    ...
                    for (
                        ...
                        nodeCurrent = getNodeState(nodeID)
                        strInfoNode += f"{nodeID}:{nodeCurrent}/{nodeOrdered},"

                    # curStartState = getNodeState(node_CtlCenter_globals.nStart)
                    # nStartStatus = node_CtlCenter_globals.dicCrossState.get(
                    #     node_CtlCenter_globals.nStart, -1
                    # )
                    # curTargetState = getNodeState(node_CtlCenter_globals.nTarget)
                    # nEndStatus = node_CtlCenter_globals.dicCrossState.get(
                    #     node_CtlCenter_globals.nTarget, -1
                    # )

                    # rospy.loginfo(
                    rospy.loginfo(
                        f"cmdIdx:{node_CtlCenter_globals.cmdIdx},Cross:{strInfoNode}CurNode:{node_CtlCenter_globals.node_current},NextNode:{node_CtlCenter_globals.node_target},cmdpos={cmd_pos},curpos={cur_pos},{node_CtlCenter_globals.listTable}"
                    )
                    # print(
                    #     f"cmdIdx:{node_CtlCenter_globals.cmdIdx},CrossStatus:{node_CtlCenter_globals.nStart},{curStartState}/{nStartStatus}|{node_CtlCenter_globals.nTarget},{curTargetState}/{nEndStatus},CurNode:{node_CtlCenter_globals.node_current},NextNode:{node_CtlCenter_globals.node_target},cmdpos={cmd_pos},curpos={cur_pos},{node_CtlCenter_globals.listTable}"
                    # )
                else:
                    rospy.loginfo(
            ...
            if node_CtlCenter_globals.waitCross:
                ...
                # 분기기를 모두 세팅해둔 후 캐리어를 이동시킨다.
                bCrossOK = True
                # 지시정보 (dicCrossState)에 맞게 분기기가 세팅되었는지 확인한다
                for nodeID, nodeOrdered in node_CtlCenter_globals.dicCrossState.items():
                    nodeCurrent = getNodeState(nodeID)
                    if (
                        nodeCurrent != nodeOrdered
                    ):  # 모든 분기기에 지시에 맞게 세팅되어야 운행.
                        bCrossOK = False
                        break
