...
import numpy as np
import pandas as pd
import roslib  # ROS 라이브러리는 일반적으로 서드 파티 라이브러리로 간주될 수 있습니다.
import rospy
import serial
from std_msgs.msg import String
import minimalmodbus

# 로컬 애플리케이션/라이브러리에 특정한 임포트
from Util import *  # '*' 사용은 가능한 피하는 것이 좋습니다.
from UtilBLB import *
from tta_blb.srv import *  # 서비스 정의 임포트, '*' 사용은 가능한 피하는 것이 좋습니다.
...
poll_START = POLL_COMMON.START.name
poll_RATE = POLL_COMMON.RATE.name
poll_LEN = POLL_COMMON.LEN.name
...
def callbackCmd(data):
    ...
    try:
        ...
        recvDataMap = getDic_strArr(recvData.upper(), sDivFieldColon, sDivItemComma)
        rospy.logdebug(recvDataMap)
        ...
        if dicMBID is None or sCMDValue is None:
            return
        ...
        """
        writeAddress = 0
        if sCMDParam is not None:  # 등록된 명령어인 경우
            ...
            for sCMDParamTmp in sCMDParamTotalSplit:  # 쉼표로 나눔 (명령어가 여러개)
                ...
                rospy.logdebug(writeValueArray)
...
def getFilePaths(dirPath, fileNames):
    """
    지정된 디렉터리 경로와 파일 이름들을 기반으로 전체 파일 경로들을 생성합니다.

    Parameters:
    - dirPath: 디렉터리의 경로
    - fileNames: 파일 이름들의 리스트

    Returns:
    - filePaths: 전체 파일 경로들의 리스트
    """
    filePaths = [f"{dirPath}/{fileName}" for fileName in fileNames]
    return filePaths

dirPath = getConfigPath(UbuntuEnv.ITX.name)
if dirPath is None:
    print(f"dirPath is None, so set to {dirPath}")
...
def Setup():
    ...
    for mbid, mbName in dicConfigTmp.items():
        ...
        try:
            ...
            filePath_modbus_cmd = f"{dirPath}/{mbName}.txt"
            ...
            bMotorOK = False
            for iTryCheckModbus in range(3):
                try:
                    if mbName.startswith(
                        "RS"
                    ):  # RS로 시작하면 서보모터여서 별도의 초기화 필요
                        # ICS 모터인 경우 리딩 매핑작업이 되어있는지 검사 (0f10 값이 0 이면 공장출하값)
                        # 공장출하상태시 WSETUP 커맨드 날림.
                        # 예외처리 할 것.
                        checkAddr = poll485[0].get("START", "-1")
                        checkAddrVal = int(checkAddr, 16)
                        checkMBresult = instrumentH.read_registers(checkAddrVal, 1, 3)
                        print(checkMBresult)
                        pub_kpalive = rospy.Publisher(
                            f"{TopicName.MB_.name}{mbid}", String, queue_size=10
                        )
                    ...
                    rospy.loginfo(
                        f"Check MB({mbid}) result Error at try {iTryCheckModbus+1} : {e}"
                    )
                    time.sleep(0.5)
...
class MotorCtl:
    ...
    def MotorMove(self, req):
        ...
        # )
        isabsolute = True
        if req_mode == f'{ServoParam.MOVE_TYPE_REL.value}':
            isabsolute = False
        ...
        # respID=getDateTime()
        resp = modbus_motorResponse(f'{respID}')
        ...
        for modbusID in dic_485poll.keys():  # 장치ID 별로 엑세스
            ...
            for dicRecord in cmdData:
                ...
                try:
                    ...
                    for itemName, itemLenStr in dicRecord.items():
                        ...
                        if strBMSDataPartBit.len >= readBytes:
                            ...
                            if itemName in dicParamParse.keys():
                                ...
                                dicModbus[keyName] = return485data[keyName] = (
                                    f"{FinalValue :.1f}"
                                )
                            else:
                    ...
                    rospy.loginfo(
                        f"{dicConfigTmp[modbusID]}:{poll_id}-{e} : Count:{exception_read_count}"
                    )
            ...
            if len(return485data) > 0:
                ...
                motorFlag = return485data.get(
                    f"{MonitoringField.ST_CMD_FINISH.name}", None
                )
                lsAckList = dic_485ack.get(int485id, None)
                ...
                if len(lsAckList) > 0:
                    ...
                    if isTimeExceeded(datetimeobj, 500):
                        if motorFlag == 1:
                            rospy.loginfo(
                                f"Popping Ack : {int485id} - {dic_485ack}, MotorFlag = {motorFlag}"
                            )
