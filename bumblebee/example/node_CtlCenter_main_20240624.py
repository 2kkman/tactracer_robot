...
if __name__ == "__main__":
    ...
    for id in ModbusID:
      sendInit = getMotorSimpleCmdString(id.value, MotorCmdField.WSTOP)
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            if not bInitOK:
                if len(node_CtlCenter_globals.dic_485ex) == node_CtlCenter_globals.numSubTopics - 2:
                    callData = String()
                ...
                if len(getRunningMotorsBLB()) == 0 and isTimeExceeded(
                    ...
                    try:
                        ...
                        # 다중제어로 한 리스트에는 복수 모터의 지시정보가 들어있고 동시에 실행시킨다.
                            ...
                            isScanOn = node_CtlCenter_globals.dicARUCO_Result.get(
                            ...
                            for dicTmp in dicInfo_local:
                                ...
                                if len(dicTmp) == 0:
                                    continue
                                arucoTableID = (
                                    node_CtlCenter_globals.dicARUCO_Result.get(
                                ...
                                if node_CtlCenter_globals.node_current not in [
                                    ...
                                    if arucoTableID == -1:
                                        ...
                                        raise Exception(sErrMsg)  # 예외를 발생시킴
                                        continue
                                rospy.loginfo(f'{type(dicTmp)},{dicTmp},Aruco:{node_CtlCenter_globals.dicARUCO_Result}')                                
                            ...
                            rospy.loginfo(type(dicTmp), dicTmp)
                            sErrMsg = f"지시정보 생성 오류 발생 : 진입해서는 안될 부분 - {dicTmp}"
                            rospy.loginfo(sErrMsg)
                            raise Exception(sErrMsg)  # 예외를 발생시킴
                        # 공통부분
