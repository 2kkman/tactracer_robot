...
sExcelPath = f'/root/SpiderGo/{datetime.datetime.now().year}.xlsx'
...
dic_CurrentNode = {}
dic_485 = {}
utilard_keepalive = datetime.datetime.now()
...
lastBMSPollingTime = datetime.datetime.now()
lastLD06Time = datetime.datetime.now()
...
def SendFeedbackFinish(strStatus:str,cmdIDtoFinish:str) -> bool:
    ...
    if cmdIDtoFinish != '' and cmdIDtoFinish != None:
        lastActionTimeTmp= datetime.datetime.now()
        
        if strStatus == CmdStatus.Finished.name:
            SetLastActionTime()
        lastActionTimeStr = str(lastActionTimeTmp).replace(':','').replace('-','')
        currentTimeStr = str(datetime.datetime.now()).replace(':','').replace('-','')
...
lastUpdateBMS = datetime.datetime.now()
def getSERVOData(instrumentCurrent:minimalmodbus.Instrument) -> list:
    ...
    try:
        ...
        pass
    td = datetime.datetime.now() - lastUpdateBMS
...
def motorStop(drvID, bSmooth):
    ...
    global lastLD06Time
    global mapping
    td = datetime.datetime.now() - lastStopTime
                ...
                if not suspend  and len(CMD_Queue) ==0: #회피기동중인 경우 대기한다.
                    ...
                    if len_df_fromSPGMAP > currOperationPos and cmdIDCurrent == '':
                        ...
                        #     SendFeedbackFinish(CmdStatus.Canceled.name,sLastSeen)
                            ...
                            if currOperationPos > 0 and sMovePrev_ == dirCaption_D and not isTrue(sWAVESCAN) and curLocV != '0' and sStart != 'RESET':
                            ...
                            if  isTrue(sSTOPWAVE):
                                if len_df_fromSPGMAP != currOperationPos+2:
                                    ...
                                    curPostMM = curLocV_INT / param_VEncoderPulse
                                    CamFileNamePrefix(f'{sLastSeen}_{(int)(curPostMM)}_{(int)(datetime.datetime.now().timestamp())}')
                                    time.sleep(2)
                                    CamEnable(True)
                                    CamLightEnable(True)
                                    time.sleep(2)
