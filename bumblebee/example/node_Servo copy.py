...
def SaveExcel():
    ...
    resultSave = False
    try:
        arrExcel.append(dicReport)
        df_stat = pd.DataFrame(arrExcel)
        #데이터프레임 엑셀파일로 추출
...
def LoadExcel():
    global arrExcel
    df = pd.read_excel(sExcelPath, engine = "openpyxl")
    arrExcel.clear()
...
def getSERVOData(instrumentCurrent:minimalmodbus.Instrument) -> list:
    ...
    #             InitBMS()
    #         else:
    #             dic_485 = getMergedDic(dic_485,BMS_Stat)
    #     else:
    #         rospy.loginfo(f'BMS Polling Error : {resultBMS}')
# else:
#     rospy.set_param(nameof(param_BMS_Show), param_BMS_Show)
    except Exception as ex:
        rospy.loginfo (f'{ex} - Try Failed in Read\r\n{traceback.format_exc()}\r\n')
        time.sleep(write_delay*1)
    return resultList
                ...
                if not suspend  and len(CMD_Queue) ==0: #회피기동중인 경우 대기한다.
                    ...
                    if len_df_fromSPGMAP > currOperationPos and cmdIDCurrent == '':
                        ...
                        #     SendFeedbackFinish(CmdStatus.Canceled.name,sLastSeen)
                            ...
                            if currOperationPos > 0 and sMovePrev_ == dirCaption_D and not isTrue(sWAVESCAN) and curLoc_V != '0':
