...
def callbackTopic2mqtt(data):
    ...
    try:
        ...
        if sTOPIC_MQTT.startswith(mqtt_topic_IMU):
            ...
            data_out["TIMESTAMP"] = getCurrentTime(spliter="")
            nodeID = sTOPIC_MQTT.split("_")[-1]
            if sTOPIC_MQTT.find("CALLBELL") > 0:
...
# listBLB = getSeqMap(node_current,4)
# print(f'최적경로:{listBLB}')
# print('END')

# print(CheckAllKeysExist(SeqMapField, listBLB[0]))
# for dicMapTmp in listBLB:
#   nStart = dicMapTmp[SeqMapField.START_NODE.name]
#   nStartStatus = dicMapTmp[SeqMapField.START_STATUS.name]
#   nTarget = dicMapTmp[SeqMapField.END_NODE.name]
#   nEndStatus = dicMapTmp[SeqMapField.END_STATUS.name]
#   nEncoder = dicMapTmp[SeqMapField.DISTANCE.name]
#   nDirection = dicMapTmp[SeqMapField.DIRECTION.name]
...
class CtlCenter:
    def __init__(self):
        ...
        self.CMD_service = rospy.Service(
            "SetCross", Kill, self.SendCMD
        )  # nodeFrom:str, nodeTo:str, crossID:str
        self.Set_service = rospy.Service(
            "SetServingTables", Kill, self.SendCMD
        )  # robotID:str, nodeID:str, orderID:str
        self.Route_service = rospy.Service(
            "GetRoute", Kill, self.SendCMD
        )  # robotID:str, tableNo:str
...
# print(GetLastString('가`나`다`라', sDivEmart))
...
rospy.Subscriber(TopicName.RFID.value, String, callbackRFID)
...
def setNodeState(nodeID, statusVal):
    ...
    if statusVal == 0 or statusVal == 1:
        ...
        SendMsgToMQTT(pub_topic2mqtt, mqttTopic, f"E:{statusVal}")
...
def RFIDControl(enable):
    rfid_B = service_setbool_client(ServiceList.NEST_RFID_INV_B.value, enable, SetBool)
    rfid_F = service_setbool_client(ServiceList.NEST_RFID_INV_F.value, enable, SetBool)
    print(rfid_B, rfid_F)
    return rfid_B


def SendCMDArd(enable):
    return service_setbool_client(ServiceList.CMDARD.value, enable, Kill)
...
if __name__ == "__main__":
    rospy.init_node(f"node_CtlCenter", anonymous=False)
