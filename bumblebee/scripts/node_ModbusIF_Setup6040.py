...
filePath_modbusconfig = f"{dirPath2}/ModbusConfig_DEV.txt"
                ...
                try:
                    if mbName.startswith(
                        ...
                        if mbName.startswith("RS6"):
                            initMotorSetup(mbid)
                            #initMotorCustom(mbid, MotorCmdField.WSETUP2)
        ...
        for modbusID in dic_485poll.keys():  # 장치ID 별로 엑세스
            ...
            for dicRecord in cmdData:
                ...
                if poll_START not in dicRecord.keys():
                    start_addr = 0
                start_addr_str = str(dicRecord[poll_START])
                if start_addr_str.startswith('#'):
                    continue
